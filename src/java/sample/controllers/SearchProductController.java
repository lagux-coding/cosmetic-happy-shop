...
public class SearchProductController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<CosmeticDTO> list = dao.getListCosmetic(search);
            List<CosmeticDTO> allCosmestic = dao.getListCosmetic();
            HttpSession session = request.getSession();
            if (search != "") {
                if (list.size() > 0) {
                    session.setAttribute("LIST_COSMETIC", list);
                }
            } else {
                session.setAttribute("LIST_COSMETIC", allCosmestic);
            }
            url = SUCCESS;
        } catch (Exception e) {
