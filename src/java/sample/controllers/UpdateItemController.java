            ...
            if(session!=null){
                ...
                if(cart!=null){
                    if(cart.getCart().containsKey(id)){
                        CosmeticDTO cosmetic = cart.getCart().get(id);
                        cosmetic.setProductQuantity(quantity);
                        boolean checkUpdate = cart.update(id, cosmetic);
                        if(checkUpdate){
                            session.setAttribute("CART", cart);
                            request.setAttribute("MESSAGE", "Update item successfully!!!");
                            url = SUCCESS;
                        }
                    }
                }else{
                    request.setAttribute("MESSAGE", "No item exist");
                }
            }else{
